...
create(Name,Cookie,Hosts,Status)->
    Record=#?RECORD{
		    name=Name,
		    cookie=Cookie,
		    hosts=Hosts,
...
read_all() ->
	...
	{aborted,{Reason,Info}}->
	    {aborted,{Reason,Info}};
	Z->
	    [{Name,Cookie,Hosts,Status}||{?RECORD,Name,Cookie,Hosts,Status}<-Z]
    end.
...
read(Object)->
    ...
    Result=case Z of
		  ...
		  [];
	       _->
		   [Info]=[{Name,Cookie,Hosts,Status}||{?RECORD,Name,Cookie,Hosts,Status}<-Z],
		   Info
	   end,
    Result.
